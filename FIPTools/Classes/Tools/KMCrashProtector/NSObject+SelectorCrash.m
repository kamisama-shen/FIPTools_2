...
id addMethod(id self, SEL _cmd) {
    NSLog(@"KMCrashProtector: unrecognized selector: %@", NSStringFromSelector(_cmd));
    return 0;
}
...
+ (void)km_enableSelectorProtector {
    ...
    dispatch_once(&onceToken, ^{
        ...
        [object km_instanceSwizzleMethod:@selector(forwardingTargetForSelector:) replaceMethod:@selector(km_forwardingTargetForSelector:)];
    });
}

- (id)km_forwardingTargetForSelector:(SEL)aSelector {
