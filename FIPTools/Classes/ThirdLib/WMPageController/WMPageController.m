...
@interface WMPageController () <NSCacheDelegate>{
    CGFloat _targetX;
...
- (void)setSelectIndex:(int)selectIndex {
    ...
    if (self.menuView && _hasInited) {
        [self.menuView selectItemAtIndex:_selectIndex];
    } else {
        _markedSelectIndex = selectIndex;
        UIViewController *vc = [self.memCache objectForKey:@(_selectIndex)];
        if (!vc) {
            vc = [self initializeViewControllerAtIndex:_selectIndex];
            [self.memCache setObject:vc forKey:@(_selectIndex)];
        }
...
- (void)wm_layoutChildViewControllers {
    ...
    if (self.isLoopScrolling) {
        if (currentPage == 0) {
            self.scrollFlag = YES;
            [self.scrollView setContentOffset:CGPointMake(_contentViewFrame.size.width * (self.childControllersCount - 1), self.scrollView.contentOffset.y) animated:NO];
            currentPage = (int)self.childControllersCount - 1;
        } else if(currentPage == self.childControllersCount - 1) {
            self.scrollFlag = YES;
            [self.scrollView setContentOffset:CGPointMake(_contentViewFrame.size.width, self.scrollView.contentOffset.y) animated:NO];
            currentPage = 0;
        }
    }
