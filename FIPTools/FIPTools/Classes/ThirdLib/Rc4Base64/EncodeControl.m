...
NSString *const kSecretKey = @"AkCCy2mcA0Rhj+mN";
@implementation EncodeControl

+ (NSString *)encode:(NSString *)data{
    return [self encode:data key:kSecretKey];
}

+ (NSString *)decode:(NSString *)data{
  return [self decode:data key:kSecretKey];
}
...
{
    NSString *str = [EncodeControl decode:data key:kSecretKey];
...
+(NSString *)encodeData:(NSDictionary *)data
{
    ...
    NSString *jsonstr = [[NSString alloc] initWithData:sendData encoding:NSStringEncodingConversionAllowLossy];
    jsonstr = [EncodeControl encode:jsonstr key:kSecretKey];
    return jsonstr;
}
