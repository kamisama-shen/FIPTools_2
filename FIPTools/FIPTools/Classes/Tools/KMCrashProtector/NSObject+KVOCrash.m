...
- (BOOL)km_addObserver:(id)object KVOinfo:(WOCPKVOInfo *)KVOinfo
{
    [self lock];
...
- (void)km_removeObserver:(id)object keyPath:(NSString *)keyPath block:(void (^)(void))block
{
//    if (!object || !keyPath) {
//        return;
//    }
    ...
    if (info != nil) {
        ...
        [KMCrashLog printCrashMsg:[NSString stringWithFormat:@"Cannot remove an observer %@ for the key path '%@' from %@ because it is not registered as an observer.",object,keyPath,self]];
    }
    [self unlock];
}
...
+ (void)km_enableKVOProtector {
    ...
    dispatch_once(&onceToken, ^{
        ...
        [objc km_instanceSwizzleMethod:@selector(addObserver:forKeyPath:options:context:) replaceMethod:@selector(km_addObserver:forKeyPath:options:context:)];
        [objc km_instanceSwizzleMethod:@selector(removeObserver:forKeyPath:) replaceMethod:@selector(km_removeObserver:forKeyPath:)];
    });
}
 ...
 */
- (void)km_addObserver:(NSObject *)observer forKeyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options context:(void *)context {
    
//    WOCPKVOInfo * kvoInfo = [[WOCPKVOInfo alloc] initWithKeyPath:keyPath options:options context:context];
//    __weak typeof(self) wkself = self;
//
//    if ([self.KVOProxy km_addObserver:wkself KVOinfo:kvoInfo]) {
//        [self km_addObserver:self.KVOProxy forKeyPath:keyPath options:options context:context];
//    }else {
//        NSLog(@"KVO is more");
//    }
//    [self km_addObserver:observer forKeyPath:keyPath options:options context:context];
    ...
    @finally {
        ...
        if (observers && [observers containsObject:observer]) {
            //        [KMCrashLog printCrashMsg:[NSString stringWithFormat:@"CrashProtector: Repeat adding the same keyPath observer: %@, keyPath: %@", observer, keyPath]];
            return;
        }
        if (!observers) {
...
- (void)km_removeObserver:(NSObject *)observer forKeyPath:(NSString *)keyPath {
//    NSLog(@"swizzled removeObserver");
//    [self.KVOProxy km_removeObserver:observer keyPath:keyPath block:^{
//        [self km_removeObserver:observer forKeyPath:keyPath];
//    }];
    ...
    @finally {
        ...
        if (!observers) {
//            [KMCrashLog printCrashMsg:[NSString stringWithFormat:@"Cannot remove an observer %@ for the key path '%@' from %@ because it is not registered as an observer.",observer,keyPath,self]];
            return;
        }
        // 重复删除观察者
